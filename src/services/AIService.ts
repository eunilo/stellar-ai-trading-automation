...
export class AIService {
  ...
  private async predictPrice(marketData: MarketData): Promise<AIPrediction> {
    try {
      ...
      const firstValue = (predictedPrice as any)[0] ?? 0;
  ...
  private prepareInputData(marketData: MarketData): tf.Tensor {
    ...
    const features = [
      ...
      Number(marketData.volume ?? 0),
      Number(marketData.change24h ?? 0),
      Number(marketData.high24h ?? 0),
      Number(marketData.low24h ?? 0)
    ];

    const data3d: number[][][] = [features.map(v => [v])];
    return tf.tensor3d(data3d, [1, features.length, 1]);
  }
  ...
  private async calculateTechnicalIndicators(_symbol: string): Promise<TechnicalIndicator[]> {
  ...
  private prepareIndicatorData(indicators: TechnicalIndicator[]): tf.Tensor {
    const values = indicators.map(ind => Number(ind.value ?? 0));
    const data3d: number[][][] = [values.map(v => [v])];
    return tf.tensor3d(data3d, [1, values.length, 1]);
  }
