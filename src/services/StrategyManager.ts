...
export class StrategyManager extends EventEmitter {
  ...
  public async monitorStrategies(): Promise<void> {
    try {
      ...
      // Simula coleta de taxa após varredura de estratégias
      const feeAmount = 0.01; // exemplo
      const feeToken = 'XLM';
      await this.soroswapService.collectTransactionFee(feeAmount, feeToken);
    } catch (error) {
      logger.error('Failed to monitor strategies:', error);
    }
  }
  ...
  async resumeStrategy(id: string): Promise<void> {
    ...
    strategy.status = StrategyStatus.ACTIVE;
    strategy.updatedAt = new Date();
    logger.info(`Strategy resumed: ${id}`);
    this.emit('strategy_update', strategy);
  }
