...
// POST /api/strategies/pause (body)
router.post('/pause', [
  body('id').isString().notEmpty().withMessage('Strategy ID is required')
], async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
// POST /api/strategies/resume (body)
router.post('/resume', [
  body('id').isString().notEmpty().withMessage('Strategy ID is required')
], async (req: Request, res: Response) => {
  try {
    ...
    const response: APIResponse<null> = {
      ...
      message: error instanceof Error ? error.message : 'An error occurred while resuming strategy',
      timestamp: new Date()
    };
